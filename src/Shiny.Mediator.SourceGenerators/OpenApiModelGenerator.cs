...
public class OpenApiModelGenerator(MediatorHttpItemConfig config, SourceProductionContext context, Compilation compilation, HashSet<string>? sharedGeneratedTypes = null)
{
    readonly string accessor = config.UseInternalClasses ? "internal" : "public";
    readonly HashSet<string> generatedTypes = sharedGeneratedTypes ?? new();

    public HashSet<string> GeneratedTypes => generatedTypes;
    ...
    void GenerateEnum(string enumName, IOpenApiSchema schema)
    {
        ...
        if (!generatedTypes.Add(enumName))
            return;
