...
public class MediatorSourceGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(combined, static (spc, source) => 
            Execute(source.Left.Left.Left.Left, source.Left.Left.Left.Right, source.Left.Left.Right, source.Left.Right, source.Right, spc));
    }
    ...
    static void Execute(ImmutableArray<HandlerInfo> handlers, ImmutableArray<MiddlewareInfo?> middleware, string rootNamespace, string? requestExecutorClassName, string? streamRequestExecutorClassName, SourceProductionContext context)
    {
        ...
        var registryCode = GenerateModuleInitializer(validHandlers, validMiddleware, namespaceName, safeNamespaceName, requestHandlers.Count > 0, streamHandlers.Count > 0, executorClassName, streamExecutorClassName);
    ...
    static string GenerateModuleInitializer(List<HandlerInfo> handlers, List<MiddlewareInfo> middleware, string? namespaceName, string safeNamespaceName, bool hasRequestHandlers, bool hasStreamHandlers, string executorClassName, string streamExecutorClassName)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated>");
        sb.AppendLine("// Code generated by Shiny Mediator Source Generator.");
        ...
        sb.AppendLine(Constants.GeneratedCodeAttributeString);
        sb.AppendLine("internal static class __ShinyMediatorRegistry");
        sb.AppendLine("{");
        ...
        foreach (var mw in middleware)
        {
            ...
            if (isOpenGeneric)
            {
                ...
                sb.AppendLine($"            builder.Services.{methodName}(typeof({interfaceName}<{typeParams}>), typeof({classTypeName}<{typeParams}>));");
            }
            else
            {
                ...
                if (mw.MiddlewareType is "Request" or "Stream")
                {
                    ...
                    sb.AppendLine($"            builder.Services.{methodName}<{interfaceName}<{requestTypeFull}, {resultTypeFull}>, {classTypeFull}>();");
                }
                else
                {
                    var requestTypeFull = GetFullTypeName(mw.RequestType);
                    sb.AppendLine($"            builder.Services.{methodName}<{interfaceName}<{requestTypeFull}>, {classTypeFull}>();");
                }
            }
        }
        ...
        if (hasRequestHandlers)
            sb.AppendLine($"            builder.Services.AddSingleton<global::Shiny.Mediator.Infrastructure.IRequestExecutor, global::{executorNamespace}{executorClassName}>();");
        
        if (hasStreamHandlers)
            sb.AppendLine($"            builder.Services.AddSingleton<global::Shiny.Mediator.Infrastructure.IStreamRequestExecutor, global::{executorNamespace}{streamExecutorClassName}>();");
