...
{
    private static readonly DiagnosticDescriptor StreamRequestInvalidHttpMethodRule = new(
        id: "SHINY001",
        title: "Invalid HTTP method for stream request",
        messageFormat: "Stream requests only support GET and POST HTTP methods. {0} is not supported for endpoint '{1}'",
        category: "Shiny.Mediator",
    ...
    {
        // UriTemplate is the first constructor argument
        var uriTemplate = attribute.ConstructorArguments.Length > 0
        ...
        {
            if (namedArg.Key == "OperationId")
            {
                operationId = namedArg.Value.Value?.ToString() ?? "";
            }
    ...
    {
        // First, validate all stream request endpoints and report diagnostics
        foreach (var classInfo in classes)
        {
            if (classInfo.IsStreamRequestHandler)
            {
                foreach (var attribute in classInfo.HttpAttributes)
                {
                    ...
                    {
                        var location = attribute.Location ?? Location.None;
                        var diagnostic = Diagnostic.Create(
                            StreamRequestInvalidHttpMethodRule,
                            location,
                            attribute.HttpMethod.ToUpper(),
    ...
    static void GenerateStreamRequestEndpoint(StringBuilder sb, string httpMethod, string uriTemplate, string requestType,
        string resultType, AttributeInfo attribute, GroupAttributeInfo? groupAttribute)
    {
        // Stream requests only support GET and POST
        // Validation and diagnostic reporting is done earlier, so just skip generation here
        if (httpMethod != "get" && httpMethod != "post")
        {
            return;
        }
    ...
    static void ApplyAttributeOnlyConfiguration(StringBuilder sb, AttributeInfo attribute)
    {
        ...
        if (attribute.Properties.TryGetValue("RequiresAuthorization", out var requiresAuth) && (bool)requiresAuth)
        {
            if (attribute.Properties.TryGetValue("AuthorizationPolicies", out var policies) &&
                policies is string[] { Length: > 0 } policyArray)
            {
                foreach (var policy in policyArray)
                {
                    sb.AppendLine($"            .RequireAuthorization(\"{policy}\")");
